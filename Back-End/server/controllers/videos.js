...
export const uploadVideo = async (req, res) => {
  try {
    ...
    const finalCorners = [
      [cornerXone, cornerYone],
      [cornerXtwo, cornerYtwo],
      [cornerXthree, cornerYthree],
      [cornerXfour, cornerYfour],
    ];
    ...
    await pool.query(
      "INSERT INTO videos (idUsuario, idCloudinary, urlVideo, urlMiniatura, titulo, rival, tipo, esFavorito, FechaPartido, arrayPiques) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
      [
        idUsuario,
        idCloudinary,
        rutaCloudinary,
    ...
    const HeatmapData = await uploadCloudinary(
      `../../Back-End-Flask/heatmap.jpg`
    );
    console.log(HeatmapData);
    const urlHeatmap = HeatmapData.url;

    // await pool.query(
    //   "INSERT INTO videos (idUsuario, idCloudinary, urlVideo, urlMiniatura, titulo, rival, tipo, esFavorito, FechaPartido, arrayPiques, urlHeatmap, velocidades) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    //   [
    //     idUsuario,
    //     idCloudinary,
    //     rutaCloudinary,
    ...
    await pool.query(
      "UPDATE videos SET arrayPiques = '?', urlHeatmap = ?, velocidades = '?' WHERE idCloudinary = ?",
      [puntosPique, urlHeatmap, velocidad, idCloudinary]
    );
...
export const deleteVideo = async (req, res) => {
  ...
  await pool.query(
    "DELETE FROM videos WHERE idUsuario = ? AND idCloudinary = ?",
    [idUsuario, idCloudinary]
  );
...
export const updateVideo = async (req, res) => {
  ...
  await pool.query(
    "UPDATE videos SET (titulo, rival, esPartido, esFavorito, FechaPartido) = (?, ?, ?, ?, ?) WHERE idUsuario = ? AND idCloudinary = ?",
    [
      titulo,
      rival,
      esPartido,
...
export const filterVideo = async (req, res) => {
  ...
  const [datosVideo] = await pool.query(
    "SELECT * from videos WHERE idUsuario = ? AND titulo LIKE CONCAT(?, '%')",
    [idUsuario, searchValue]
  );
  const [datosEntrenamientos] = await pool.query(
    "SELECT * from videos WHERE idUsuario = ? AND tipo = 'Entrenamiento' AND titulo LIKE CONCAT(?, '%')",
    [idUsuario, searchValue]
  );
  const [datosPartidos] = await pool.query(
    "SELECT * from videos WHERE idUsuario = ? AND tipo = 'Partido' AND titulo LIKE CONCAT(?, '%')",
    [idUsuario, searchValue]
  );
  ...
  res
    .status(200)
    .json({ datosVideo, cantVideos, cantEntrenamientos, cantPartidos });
};
...
export const getVideos = async (req, res) => {
  ...
  const [datosVideo] = await pool.query(
    "SELECT * from videos WHERE idUsuario = ?",
    [idUsuario]
  );
  const [datosEntrenamientos] = await pool.query(
    "SELECT * from videos WHERE idUsuario = ? AND tipo = 'Entrenamiento'",
    [idUsuario]
  );
  const [datosPartidos] = await pool.query(
    "SELECT * from videos WHERE idUsuario = ? AND tipo = 'Partido'",
    [idUsuario]
  );
  const [datosUser] = await pool.query(
...
export const getVideo = async (req, res) => {
  ...
  const [datosVideo] = await pool.query(
    "Select urlVideo, arrayPiques, urlHeatmap, velocidades, titulo, tipo FROM videos WHERE idUsuario = ? AND idcloudinary = ?",
    [idUsuario, idCloudinary]
  );
  ...
  const tipo = datosVideo[5];
  return res
    .status(200)
    .json(urlVideo, arrayPiques, urlHeatmap, velocidades, titulo, tipo);
};
