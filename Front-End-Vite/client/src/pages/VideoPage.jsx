...
function VideoPage() {
    ...
    setInterval(() => {
      if (!paused) {
        ...
        const drawCircle = (pt) => {
          circle.beginPath();
          circle.arc(
            parseInt(pt[0][0] / 15) + 51,
            parseInt(pt[0][1] / 15) + 25,
            3.5,
            0,
            Math.PI * 2
          );
  ...
  useEffect(() => {
    const getVideo = async () => {
      const videoData = await axiosPrivate.get(`/GetVideo/${id}/${idVideo}`);
          ...
          <div className="stats">
            <div className="minimap-container">
              <div className="title-map">
                <IconButton onClick={handleBack} disabled={mapActive === 0}>
                  {/* id={mapActive  === 0 ? "left-chevron" : ""} */}
                  <ChevronLeftIcon />
                </IconButton>
                <div className="title">
                  {" "}
                ...
                <IconButton onClick={handleNext} disabled={mapActive === 1}>
                  {" "}
                  {/*  id={mapActive  === 1 ? "right-chevron" : ""} */}
                  <ChevronRightIcon />
                </IconButton>
              </div>
              <div className="minimap">
                ...
                {/*points*/}
                <div
                  className={mapActive === 1 ? "heat heat-active" : "heat"}
                  style={{ backgroundImage: `url(${urlHeatmap})` }}
                ></div>{" "}
                {/*heat*/}
              </div>
